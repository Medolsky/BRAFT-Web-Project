            ...
            {filteredItems.map((item, idx) => (
              <ScaleIn key={item.id} delay={idx * 0.1}>
                <Card className="p-0 overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-0 border-white/10 hover:border-purple-500/30 group">
                  <SlideIn direction="left" className={`lg:col-span-7 bg-zinc-950 aspect-[16/10] overflow-hidden flex items-center justify-center border-b lg:border-b-0 lg:border-r border-white/5 ${
                    item.thumbnailUrl.includes('braft-logo') ? 'p-8 sm:p-12' : 'p-0'
                  }`}>
                    <img
                      src={item.thumbnailUrl}
                      alt={item.title}
                      className={
                        item.thumbnailUrl.includes('braft-logo')
