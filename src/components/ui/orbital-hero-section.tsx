...
export function OrbitalHeroSection({
  ...
  useEffect(() => {
    ...
    function elementsOf(
      ...
      const fan = PLANE_FAN[index % PLANE_FAN.length] || [0, 0];
      const inc = (p.i + spread * fan[0]!) * RAD;
      const node = (p.node + spread * fan[1]!) * RAD;
      const target = ECC_FAN[index % ECC_FAN.length] || 0.2;
    ...
    function syncElements() {
      ...
      const apex0 = C.apex ? C.apex[0] : 272;
      const apex1 = C.apex ? C.apex[1] : 53;
      const key =
        C.compress + "/" + C.planeSpread + "/" + C.eccentricity + "/" +
        C.alignToCourse + "/" + apex0 + "," + apex1 + "/" +
    ...
    function render(t: number) {
      ...
      const fx = C.focus ? C.focus[0] : 0.5;
      const fy = C.focus ? C.focus[1] : 0.5;
      cx = width * fx;
      cy = height * fy;
      ...
      for (let s = 0; s < starN; s++) {
        ...
        const starY = sy[s] ?? 0;
        const starZ = sz[s] ?? 0;
        project(starX - DIR.x * dist, starY - DIR.y * dist, starZ - DIR.z * dist);
        ...
        const mag = sMag[s] ?? 0.5;
        const phase = sPhase[s] ?? 0;
        let a = (0.2 + mag * 1.05) * Math.pow(dRef / P.depth, 0.8) * near * far;
        if (a <= 0.012) continue;
        a *= 0.82 + 0.18 * Math.sin(t * 9 + phase);
        const tint = sTint[s] ?? 0;
        const col = tint === 1 ? "175,205,255" : tint === 2 ? "255,214,170" : "255,255,255";
        const size = Math.min(2.3, 0.55 + mag * 1.5 * Math.pow(dRef / P.depth, 0.5));
      ...
      let idx = 0;
      while (idx < shots.length && shots[idx] && shots[idx]!.depth > camDist) {
        drawShot(shots[idx]!);
        idx++;
      }
      drawSun(k, t);
      while (idx < shots.length && shots[idx]) {
        drawShot(shots[idx]!);
        idx++;
      }
